...
export const DeleteSelectedCategoriesDialog = () => {
    ...
    startTransition(async () => {
      if (dialogData) {
        dialogData.map(async (item) => {
          deleteCategoryById(params.storeId[0], item.id)
            .then(async (data) => {
              if (data.error) {
                setError(data.error)
              }
  ...
  return (
    <Dialog
      open={dialog.isOpen && dialog.name === 'deleteSelectedCategories'}
